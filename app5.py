...
pmc_total = ct + ck  # Total marginal propensity to consume

if pmc_total >= 1:
    st.warning("⚠️ La propensión marginal a consumir (ct + ck) es ≥ 1. El multiplicador es infinito o indefinido.")
    st.metric("Multiplicador Keynesiano", "∞ (Infinito)")
elif pmc_total < 0:
    st.error("❌ La propensión marginal a consumir no puede ser negativa.")
else:
    multiplier = 1 / (1 - pmc_total)
    st.metric("Multiplicador Keynesiano", f"{multiplier:.2f}")
    
    # Optional: Show what this means
    with st.expander("¿Qué significa el multiplicador?"):
        st.write(f"""
        Un multiplicador de {multiplier:.2f} significa que por cada peso 
        adicional de gasto autónomo (inversión, gasto público, etc.), 
        el PIB aumentará en {multiplier:.2f} pesos.
        """)
