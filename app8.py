...
# Initialize master full_cycles if not present
if "stored_year_values" not in st.session_state:
    st.session_state["stored_year_values"] = {year: {name: None for name in funcs} for year in [2024, 2025]}
...
for name, (func, p1, p2, x_vals, xlabel, ciclo_func) in funcs.items():
    ...
    with col_plot2:
        ...
        # Compute current value for the last x
...
with col_store:
    store_pib_clicked = st.button("Guardar PIB calculado", key="guardar_pib_final")
...
# Construcción de los datos para graficar
years_plot = sorted(pib_fijo.keys())
values_plot = [
    st.session_state["pib_dinamico"].get(y, pib_fijo[y]) for y in years_plot
]

# Gráfica de evolución del PIB
fig_pib, ax_pib = plt.subplots(figsize=(8, 4))
ax_pib.plot(years_plot, values_plot, marker="o", color="blue", linestyle="-", label="PIB")
